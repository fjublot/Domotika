...
class onewire extends top
{
	...
	public function form($page=null) {
		...
		$return .= fn_HtmlSelectField('displayformat', 'displayformat', 'an.displayformat');
	...
	public function getxmlvalue($xmlStatus) {
		...
		if ( $this->valeur != "" )
		{
			...
			{
				$current_compensation = new onewire($this->compensation);
	...
	public function disp_list() {
		...
		$return .= '				<p class="searchable">'.ucfirst(fn_GetTranslation("onewire")).' : '.($this->no+1).' ('.$this->xmlid.')</p>';

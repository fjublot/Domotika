<?php
	...
	switch ($control) {
			...
			Case 'iswritableconfigfile':
			// Test si un fichier est accessible en écriture
				$return=is_writable ($GLOBALS["config_file"]);
				if ($return==false) {
				...
				$result[] = array(
					...
					'valid' => $return?true:false,
					'message' => $return ? fn_GetTranslation('writing_file', $GLOBALS["config_file"]) : fn_GetTranslation('no_writing_file', $GLOBALS["config_file"])
				);
				break;

<?php
	...
	*/
	...
	class model
	{
		...
		const ROWCOUNTARRAY = 4; //Retour Array + rowCount()
		...
		*/
		...
		/**
		* Cette fonction vérifie si un champ existe dans une table
		...
		*/
		...
		public function getAll($table, $order_by = '', $desc = false, $limit = false, $offset = false)
		{
			if ($this->tableExist($table))
			{
				...
				if ($limit !== false)
				{
					$req->bindParam(':limit', $limit, self::PARAM_INT);
					if ($offset !== false)
					{
						$req->bindParam(':offset', $offset, self::PARAM_INT);
					}
				}
				...
				/*
		 * Cette fonction permet de verifir l'existence d'un index sur un champ
		 */
		
		public function ExistIndexesForTable($table, $field) {
			if ($this->tableExist($table)) {
				$query = 'SHOW INDEXES FROM `'.$table.'` WHERE Key_name = :field';			
				$datas = array(
					'field' => $field
				);
			
				$indexes = $this->runQuery($query, $datas, self::ROWCOUNT);
				if ($indexes>0)
					return true;
				else
					return false;
		...
		public function tablesstatus() {
				...
				$datas = array();
				return $this->runQuery($query, $datas, self::FETCHALL);
		}
		
		public function meta($query, $datas = array(), $column_index) {
			$req = $this->bdd->prepare($query);
			$req->execute($datas);
			return $req->getColumnMeta($column_index);
		}
